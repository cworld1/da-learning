...
\begin{frame}
\center{\huge When the response is binary}
\end{frame}
...
We have already seen it! Remember odds?
\[
Odds = \frac{p}{1-p}
\]
...
\end{enumerate}
When would want to do this?
\end{frame}

\begin{frame}
\frametitle{Example -- Basketball}
In 2012 I made my STATS 340 class play basketball in class. In this experiment I:
\begin{enumerate}[-]
...
\item I recorded a \rcode{1} the shot was successful, and a \rcode{0} if the shot was missed
\end{enumerate}
\vspace{5 mm}
\textbf{Questions}\\
\begin{enumerate}[1.]
...
In general, because the responses are zeros and ones, it is hard to plot this kind of data. However we can look at the frequency table
<<R2>>=
success.tbl = xtabs(basket~distance+gender, data = bb.df)
success.pct = 100 * success.tbl / 10
success.pct
@
...
<<R10, size="scriptsize">>=
observed = rowMeans(success.pct) / 100
...
Alternatively, we can regard the response variable as a proportion.
...
Most of the crew survived the initial breakup, 
...
(approx. US\$ 6 billion in todays terms.)\footnote{Unfortunately, 
...
The temperature at the launch site was a chilly 31F.
...
It was suspected that O-ring reliabilty was influenced by temperature.
...
\end{center}
and so it was decided to approve the launch on that 31F morning.
\end{frame}
...
To estimate the probability of retention for haddock of lengths 25, 35 and 45 cm:
  ...
  <<eval=FALSE>>=
  pred35=data.frame(forklen=c(25,35,45))
predictCount(Haddock.glm,pred35)
@
  <<echo=FALSE>>=
  pred35=data.frame(forklen=c(25,35,45))
predictCount(Haddock.glm,pred35,print.out=FALSE)
@
