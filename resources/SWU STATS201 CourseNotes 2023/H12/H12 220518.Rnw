...
%\footnote{If there were no interaction between the two factors but each factor independently has an effect, we specify \rcode{specs = pairwise $\sim$ Attend} or \rcode{specs = pairwise $\sim$ Pass.test}.}
%\medskip
%
...
%<<RC-H12-017a, warning=F>>=
%library(emmeans)
%emm_options(sep = ":")
%@

The \rcode{emmeans()} function creates a list with two components. We'll store the result in \rcode{exam.emm} so that we can print each object separately.
\bigskip
...
Here, \rcode{pairwise $\sim$ Attend:Pass.test} is an \rcode{R} formula that is specifying an interaction model between \rcode{Attend} and \rcode{Pass.test}. This is telling \rcode{emmeans} that we want pairwise comparisons for every treatment combination.
\end{frame}
...
The first component of \rcode{exam.emm}, named \rcode{emmeans}, contains four rows: one per treatment\footnote{The word {\em treatment} is often used on its own to refer to a combination of the levels of two or more factor variables.} combination of the levels of \rcode{Attend} and \rcode{Pass.test}. 
\medskip
...
This is equivalent to the output from the coefficient summary table for the \rcode{Exam.fitNoBaseline} model, with the addition that \rcode{lower.CL} and \rcode{upper.CL} give the 95\% confidence interval for the treatment means.
\end{frame}
...
The second component of \rcode{exam.emm} contains the  {\em simultaneous} confidence intervals for the six\footnote{\rcode{choose(4,2)=6}} possible pairwise comparisons between the four treatment combinations.

<<RC-H12-017d, size="scriptsize">>=
exam.emm$contrasts
@
\vfill
...
\frametitle{Exam vs test success and attendance}
\framesubtitle{Pairwise comparisons\ldots}
For our Executive Summary we require simultaneous confidence intervals only for the simple contrasts\footnote{Only those pairwise comparisons which involve conditioning on the level of one of the two factors. That is, they share a common level of one of the factors.} between means. 
\medskip

We exclude rows 3 and 4 of the table since they are not simple contrasts.
\medskip

<<RC-H12-017e, echo = 1, digits=4>>=
print(data.frame(exam.emm$contrasts)[-c(3,4),], digits=4)
@

For example, the second row of the above \rcode{contrasts} table says that the estimated difference between the means of the two levels of \rcode{Pass.test} conditional on the level of \rcode{Attend} = \rcode{No} is \texttt{-13.02}. So, for students who do not regularly attend lectures, those who pass the test will score an average of 13 points higher in the exam than those who fail.
\end{frame}
...
We see that the gender is also not significant here\footnote{If it were significant then we could look at the output from \rcode{emmeans(Exam.fit3, specs = pairwise $\sim$ Attend)} and \rcode{emmeans(Exam.fit3, specs = pairwise $\sim$ Gender)} -- but this is pointless here since the factors only have two levels and there is only one pairwise comparison and so no multi-comparison issue.}, so we again apply Occam's razor and remove this term.
\end{frame}
...
exam.emm=emmeans(Exam.fit, specs = pairwise ~ Attend:Pass.test)
exam.emm$contrasts
@
...
exam.emm=emmeans(additive.fit, specs = pairwise ~ variable1)
exam.emm$contrasts
@

<<RC-H11-3f, eval=F, comment=NA>>=
exam.emm=emmeans(additive.fit, specs =  pairwise ~ variable2)
exam.emm$contrasts
@
\bigskip
