...
ModelCheck=function(Fit,which=1:3,mar=c(3,4,1.5,4)) {
  k=1 #Value of next plot
  M=matrix(NA,length(which),2)
  if(1 %in% which) { M[1,]=c(1,1); k=k+1}
  if(2 %in% which) { M[k,]=c(k,k+1); k=k+2}
  if(3 %in% which) M[length(which),]=c(k,k)
  layout(M)
  par(mar=mar,mgp=c(2,1,0))
  if(1 %in% which) plot(Fit,which=1)
  if(2 %in% which) {
    plot(Fit,which=2) 
    hist(resid(Fit),main="Histogram of residuals") }
  if(3 %in% which) plot(Fit,which=4)
}
@
...
<<MC, fig.height=2.1, fig.width=4.4>>=
par(las=1)
ModelCheck(examtest.fit,1) #plot(examtest.fit, which=1)
@

<<RC-H02-009, echo=FALSE, fig.show='hide'>>=
...
<<Test, echo=F,fig.height=5>>=
ModelCheck(examtest.fit)
@
\end{frame}

\begin{frame}
\frametitle{Model checks 2}
\vspace{-1ex}
<<Test 2, echo=F, fig.width=6, fig.asp=0.7>>=
