...
\title{Chapter 10: Multiple linear regression modelling}
...
<<RC-H10-012, fig.show='hide'>>=
pairs20x(Babies.df[,c(1,3,7)])
@

<<RC-H10-013, echo=FALSE>>=
...
Most babies are born before 42 weeks = $42 \times 7 =294$ days. It seems that beyond this point babies cease to grow and hence the `flattening out' and/or decrease. In other words, it looks like the effect of gestational age depends on whether the baby is overdue or not. That is, the effect of gestational age appears to {\bf interact} with overdue status.

\medskip
It would make sense to use overdue status as an explanatory variable. Note that gestational age is a numeric variable, and being overdue (or not) is a factor variable. We already know how to fit a model using both of these as explanatory variables. Let's create a factor variable \rcode{OD} for overdue status. 
\medskip
...
\begin{frame}[fragile]
\frametitle{Birth weight of babies\ldots}
Our initial model will be an interaction model using explanatory variables \rcode{gestation} and \rcode{OD}.
...
\frametitle{Birth weight of babies, alternate model}
We refit the alternate model using the reduced data.

<<RC-H10-027alt, fig.height=3>>=
ODtime=ifelse(Babies.df$gestation<=294,0,Babies.df$gestation-294)
bwt.fit3b=lm(bwt~ gestation+ODtime,data = Babies.df, subset = -c(239, 820))
gest.seq=200:360; ODtime.seq=ifelse(gest.seq<=294,0,gest.seq-294)
fit.seq=predict(bwt.fit3b,new=data.frame(gestation=gest.seq,ODtime=ODtime.seq))
par(mar=c(4,4,0,4)); plot(bwt~gestation,data=Babies.df[-c(239, 820),])
lines(gest.seq,fit.seq,col="red")
@
\end{frame}
...
The fitted model is:
\begin{align*}
 \E[\rcode{bwt}]&=\Sexpr{round(b[1], 2)} + \Sexpr{round(b[2], 2)}\times\rcode{gestation} + \Sexpr{round(b[3], 2)}\times\rcode{OD}  \\
                     &\quad\Sexpr{round(b[4], 2)}\times\rcode{OD}\times\rcode{gestation}
\end{align*}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{Birth weight of babies\ldots}
So, for $\rcode{gestation} \le 294$ days (i.e., \rcode{OD = 0})
\[
\E[\rcode{bwt}]=\Sexpr{round(b[1], 2)}+\Sexpr{round(b[2], 2)}\times\rcode{gestation}
\]
So, on average, babies initially grow at \Sexpr{round(b[2], 2)} oz per day until about \Sexpr{round(b[1] + b[2] * 294)} oz\footnote{
$\Sexpr{round(b[1] + b[2] * 294)} \approx \Sexpr{round(b[1], 2)} + \Sexpr{round(b[2], 2)}\times 294$} at week 42 (i.e., day 294).\\
\bigskip
For $\rcode{gestation} > 294$ days (\rcode{OD = 1})
\begin{align*}
\E[\rcode{bwt}]&=\Sexpr{round(b[1], 2)}+\Sexpr{round(b[3], 2)}+(\Sexpr{round(b[2], 2)}-\Sexpr{round(abs(b[4]), 2)})\times\rcode{gestation} \\
           &= \Sexpr{round(b[1] + b[3], 2)} - \Sexpr{round(abs(b[2] + b[4]), 2)}\times\rcode{gestation}
\end{align*}

So, on average, it is estimated that overdue babies lose about \Sexpr{round(abs(b[2] + b[4]), 2)} oz per day after week 42.\footnote{{\bf Question:} How could we test whether this is significantly different from zero?}
\end{frame}
%Babies.df$OD2=relevel(Babies.df$OD,ref="1")
%bwt.fit3b=lm(bwt~ gestation*OD2,data = Babies.df, subset = -c(239, 820))
%coef(summary(bwt.fit3b))
%confint(bwt.fit3b)
...
\begin{frame}[fragile]
\frametitle{Birth weight of babies\ldots}
Let us add \rcode{weight} to the model.\\
\bigskip
...
<<RC-H10-041, fig.show = 'hide'>>=
pairs20x(Babies.df[-c(239,820), c(9,6,5)])
@

<<RC-H10-042, echo=FALSE>>=
...
{\renewcommand{\arraystretch}{1.15}
  ...
  \begin{tabular}{l|ll}
    ...
    \rcode{bwt.fit3} & Added \rcode{gestation * OD} & \Sexpr{round(100*summary(bwt.fit3)$r.squared, 1)}\% \\
...
\begin{frame}[fragile]
\frametitle{Birth weight of babies\ldots}
\framesubtitle{Closing remarks\ldots}
...
\bigskip
Model selection is a crucial component of statistical modelling and machine learning.
...
Then it is a dialogue with your usual outputs (e.g: \rcode{summary(bwt.fit3)} and \rcode{summary(bwt.fit4)} etc...) and to see if what you observe makes sense. Note that this requires constant vigilance. 
\smallskip
